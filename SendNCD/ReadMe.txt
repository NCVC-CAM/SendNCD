========================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : SendNCD プロジェクトの概要
========================================================================


この SendNCD DLL は、AppWizard により作成されました。この DLL は MFC の基本的な使い方を示すだけでなく、DLL を作成するための開始点でもあります。

このファイルには、SendNCD DLL を構成する各ファイルの内容の概略が記述されています。
...
SendNCD.h
    これは DLL のメインのヘッダー ファイルです。CSendNCDApp クラスを宣言します。

SendNCD.cpp
    これは、メインの DLL ソース ファイルです。CSendNCDApp クラスが含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、SendNCD.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。
